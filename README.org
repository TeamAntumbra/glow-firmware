...
- LUFA (http://www.fourwalledcubicle.com/LUFA.php), extracted with top-level
  directory ~LUFA-xxxxxx/~ as ~lufa/~. Version 140302 is known to work.
...
Firmware builds may be downloaded from [[https://dev.antumbra.io/build/rawtumbra/]].
...
Since ~00 00 00 00~ and ~ff ff ff ff~ are end markers, the option ID may not be
...
sequence of 4 ASCII characters. For example, the option ID "TEST" would be
encoded as ~54 45 53 54~.
