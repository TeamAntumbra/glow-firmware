...
- =<info>=: an arbitrary string, but recommended to be in a form that uniquely
  identifies the implementation (such as reverse-DNS
  =org.example.glow-compatible-device= notation)
